...
#%%
data_dir = os.path.join(home_dir, 'data/alex_2d')
save_dir = os.path.join(home_dir, 'data/alex_2d_r') # save directory default: data_dir = 'data/alex_2d'
idxs = [1]
data = {'entries': []}
for idx in idxs:
...
#%% 
# define a function to cut redundant space of the Structure object, especiallly for 2D materials with vacuum in z-direction. 
def cut_structure(struct, z_extra=0.0):
    ...
    frac = np.array(struct.frac_coords)
    # frac_xy, = frac[:, :2]
    
    # find the min and max z-coordinates
    z_min, z_max = np.min(frac[:, 2]), np.max(frac[:, 2])
...
keys_dict = {'material_id': 'mat_id', 'formation_energy_per_atom': 'e_form', 'band_gap_ind': 'band_gap_ind', 'band_gap_dir': 'band_gap_dir', 'e_above_hull':'e_above_hull', 'pretty_formula': 'formula', 'spacegroup.number':'spg', 'spacegroup.number.conv':'spg'}
...
threshold_lower_dict = {'formation_energy_per_atom': 2.0, 'e_above_hull': 0.2, 'natm': 21}  #TODO: adjust this
