...
from scigen.pl_modules.lattice.crystal_family import CrystalFamily
...
class BaseModule(pl.LightningModule):   # BaseModule from diffCSP (copied from SCIGEN/)
...
class CSPDiffusion(BaseModule):
    def __init__(self, *args, **kwargs) -> None:
        ...
        # print('self.hparams.keys (b): ', self.hparams.keys())
        ...
        # print('self.hparams.keys (a): ', self.hparams.keys())
    ...
    def forward(self, batch, batch_idx = None):
        ...
        # rand_x_anchor = rand_x[batch.anchor_index]  #TODO
        # rand_x_anchor = (batch.ops_inv[batch.anchor_index] @ rand_x_anchor.unsqueeze(-1)).squeeze(-1)   #TODO
        # rand_x = (batch.ops[:, :3, :3] @ rand_x_anchor.unsqueeze(-1)).squeeze(-1)   #TODO
        # input_frac_coords = (frac_coords + sigmas_per_atom * rand_x) % 1.   #TODO
        ...
        # rand_t = torch.randn_like(gt_atom_types_onehot)[batch.anchor_index] #TODO
        ...
        # atom_type_probs = atom_type_probs[batch.anchor_index]   #TODO
        ...
        pred_x_proj = torch.einsum('bij, bj-> bi', batch.ops_inv, pred_x)   #TODO

        # tar_x_anchor = d_log_p_wrapped_normal(sigmas_per_atom * rand_x_anchor, sigmas_per_atom) / torch.sqrt(sigmas_norm_per_atom)   #TODO
        tar_x = d_log_p_wrapped_normal(sigmas_per_atom * rand_x, sigmas_per_atom) / torch.sqrt(sigmas_norm_per_atom)    #!

        loss_lattice = F.mse_loss(pred_crys_fam, rand_crys_fam)
        # loss_coord = F.mse_loss(pred_x_proj, tar_x_anchor)   #TODO
    ...
    def sample(self, batch, diff_ratio = 1.0, step_lr = 1e-5):
        ...
        # time_start = self.beta_scheduler.timesteps - 1  #! 241122 comment out for now
        time_start = self.beta_scheduler.timesteps

        l_T = self.crystal_family.v2m(crys_fam_T)

        # x_T_all = torch.cat([x_T[batch.anchor_index], torch.ones(batch.ops.size(0),1).to(x_T.device)], dim=-1).unsqueeze(-1) # N * 4 * 1    #TODO
        # x__T = torch.rand([batch.num_nodes, 3]).to(self.device)
        # x_T = (batch.ops @ x_T_all).squeeze(-1)[:,:3] % 1. # N * 3  #TODO
        # t_T = t_T[batch.anchor_index]   #TODO
        ...
        for t in tqdm(range(time_start, 0, -1)):
            ...
            # pred_x_proj = torch.einsum('bij, bj-> bi', batch.ops_inv, pred_x)   #TODO
            # pred_x_anchor = scatter(pred_x_proj, batch.anchor_index, dim=0, reduce = 'mean')[batch.anchor_index]    #TODO
            # pred_x = (batch.ops[:, :3, :3] @ pred_x_anchor.unsqueeze(-1)).squeeze(-1)   #TODO

            x_t_minus_05 = x_t - step_size * pred_x + std_x * rand_x

            crys_fam_t_minus_05 = crys_fam_t

            # frac_coords_all = torch.cat([x_t_minus_05[batch.anchor_index], torch.ones(batch.ops.size(0),1).to(x_t_minus_05.device)], dim=-1).unsqueeze(-1) # N * 4 * 1  #TODO
            # x_t_minus_05 = (batch.ops @ frac_coords_all).squeeze(-1)[:,:3] % 1. # N * 3  #TODO
            ...
            # rand_x_anchor = (batch.ops_inv[batch.anchor_index] @ rand_x_anchor.unsqueeze(-1)).squeeze(-1)  #TODO
            # rand_x = (batch.ops[:, :3, :3] @ rand_x_anchor.unsqueeze(-1)).squeeze(-1)  #TODO
            # rand_t = rand_t[batch.anchor_index]  #TODO
            ...
            # pred_t = scatter(pred_t, batch.anchor_index, dim=0, reduce = 'mean')[batch.anchor_index]  #TODO
            ...
            # frac_coords_all = torch.cat([x_t_minus_1[batch.anchor_index], torch.ones(batch.ops.size(0),1).to(x_t_minus_1.device)], dim=-1).unsqueeze(-1) # N * 4 * 1  #TODO

            # x_t_minus_1 = (batch.ops @ frac_coords_all).squeeze(-1)[:,:3] % 1. # N * 3  #TODO

            t_t_minus_1 = c0 * (t_t_minus_05 - c1 * pred_t) + sigmas * rand_t

            # t_t_minus_1 = t_t_minus_1[batch.anchor_index]   #TODO
        ...
        res = dc(traj[0])   #? (comment out for now)
        # res['atom_types'] = res['atom_types'].argmax(dim=-1) + 1    #?  (comment out for now)

        # return res, traj_stack  #?  (comment out for now)
...
@torch.no_grad()
def sample_scigen(self, batch, diff_ratio = 1.0, step_lr = 1e-5, stop_ratio = 0.0):     #!@
# def sample_scigen(self, batch, diff_ratio = 1.0, step_lr = 1e-5):     

    batch_size = batch.num_graphs

    x_T_unk = torch.rand([batch.num_nodes, 3]).to(self.device)  #!
    crys_fam_T = torch.randn([batch_size, 6]).to(self.device)
    crys_fam_T = self.crystal_family.proj_k_to_spacegroup(crys_fam_T, batch.spacegroup)
    t_T_unk = torch.randn([batch.num_nodes, MAX_ATOMIC_NUM]).to(self.device)    #!

    x_T_known = torch.rand([batch.num_nodes, 3]).to(self.device)       #!
    t_T_known = torch.randn([batch.num_nodes, MAX_ATOMIC_NUM]).to(self.device)  #!    

    x_0_known = batch.frac_coords_known #!
    t_0_known = F.one_hot(batch.atom_types_known - 1, num_classes=MAX_ATOMIC_NUM).float()   #!

    mask_x, mask_t = batch.mask_x, batch.mask_t   #!
    mask_t = mask_t.unsqueeze(-1)
    if len(mask_x.shape) == 1:
        print('Use reduce mask')
        mask_x = mask_x.unsqueeze(-1)
        
    mask_x_inv,  mask_t_inv = 1 - mask_x, 1 - mask_t    #!
    mask_x_stop, mask_t_stop = torch.zeros_like(mask_x), torch.zeros_like(mask_t)   #! will update later    #!@
    mask_x_inv_stop, mask_t_inv_stop = 1 - mask_x_stop, 1 - mask_t_stop   #! will update later  #!@
    ...
    time_stop = int(time_start * stop_ratio)  #!@ check later    
    switched_time = False
    ...
    for t in tqdm(range(time_start, 0, -1)):
        if t < time_stop and not switched_time:
            mask_x = mask_x_stop
            mask_t = mask_t_stop
            mask_x_inv = mask_x_inv_stop
            mask_t_inv = mask_t_inv_stop
            switched_time = True
            print(f"Since t = {t} < time_stop, mask_x, mask_t, mask_x_inv, mask_t_inv are set to 0.")
        ...
        l_t_unk = traj[t]['lattices']   #? no need?
        crys_fam_t = traj[t]['crys_fam']
        ...
        pred_crys_fam, pred_x, pred_t = self.decoder(time_emb, t_t_unk, x_t_unk, crys_fam_t, batch.num_atoms, batch.batch)  #!
        ...
        x_t_minus_05_unk = x_t_unk - step_size * pred_x + std_x * rand_x    #!
        ...
        x_t_minus_1_unk = x_t_minus_05 - step_size * pred_x + std_x * rand_x    #!
