...
target = 'band_gap_dir'    # ['ehull', 'f_energy']
descriptor = 'ie'   # ['mass', 'number', 'radius', 'en', 'ie', 'dp', 'non']
threshold = 0.4   # target threshold [eV]
ehull_threshold = 0.10   # ehull threshold [eV]
change_cells=False
num_diff = 2
...
conf_dict = make_dict([file_name,run_name,model_dir,data_dir,alex_dir,idxs_data,natm_max,csv_header,
                    ...
                    threshold,ehull_threshold,change_cells,diff_factor,num_diff, diff_factor_small, num_diff_small, max_iter, 
...
# data['stable'] = np.where(data['ehull'] < ehull_threshold, 1, 0)
if ehull_threshold is not None:
    data = data[data['ehull'] < ehull_threshold].reset_index(drop=True)
    logger.info(f"Filtered data with ehull < {ehull_threshold}: {len(data)}")
    
data['stable'] = np.where(data[target] >= threshold, 1, 0)
...
dataset = Dataset_Cls(data, r_max, 'ehull', descriptor, scaler, nearest_neighbor)  #TODO: change target to 'stable' (temporary)
num = len(dataset)
print('dataset: ', num)
