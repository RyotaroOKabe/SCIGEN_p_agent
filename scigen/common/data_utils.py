...
def build_crystal_graph(crystal, graph_method='crystalnn'):
    ...
    if graph_method == 'crystalnn':
        try:
            ...
            try:
                # First fallback: increase search_cutoff to 10
                ...
                # Second fallback: increase search_cutoff to 20 for very sparse structures
                crystalNN_tmp2 = local_env.CrystalNN(distance_cutoffs=None, x_diff_weight=-1, porous_adjustment=False, search_cutoff=20)
                crystal_graph = StructureGraph.with_local_env_strategy(
                    crystal, crystalNN_tmp2) 
...
def process_one(row, niggli, primitive, graph_method, prop_list, use_space_group = False, tol=0.01):
    try:
        ...
        # Skip structures that cannot be processed (e.g., no Voronoi neighbors found)
        # Return None and filter out later
        import warnings
        warnings.warn(f"Skipping material {row.get('material_id', 'unknown')}: {str(e)}")
        return None
...
def preprocess(input_file, num_workers, niggli, primitive, graph_method,
    ...
    # Filter out None results (structures that failed to process)
    valid_results = [result for result in unordered_results if result is not None]
    skipped_count = len(unordered_results) - len(valid_results)
    if skipped_count > 0:
        import warnings
        warnings.warn(f"Skipped {skipped_count} structures that could not be processed")
    
    mpid_to_results = {result['mp_id']: result for result in valid_results}
    # Only include results for materials that were successfully processed
    ordered_results = [mpid_to_results[df.iloc[idx]['material_id']]
                       for idx in range(len(df))
                       if df.iloc[idx]['material_id'] in mpid_to_results]
