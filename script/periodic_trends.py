...
def plotter(
    ...
    if log_scale:
        ...
        color_mapper = LogColorMapper(
            ...
            palette=bokeh_palette, low=under_value, high=over_value   #!low=min(data), high=max(data)
        )
        # norm = LogNorm(vmin=0.0, vmax=1.0)  #!LogNorm(vmin=min(data), vmax=max(data))
        norm = LogNorm(vmin=under_value, vmax=over_value)  #!LogNorm(vmin=min(data), vmax=max(data))
    else:
        ...
        norm = Normalize(vmin=under_value, vmax=over_value)    #!Normalize(vmin=min(data), vmax=max(data))
